...
finalFrontier <- function(treatment, dataset, drop, metric, mdist = NULL, breaks=NULL){
...
frontierEst <- function(frontierObject, dataset, myform=NULL, treatment=NULL, estCall=NULL, drop=NULL){
  ...
  if(frontierObject$metric=="Mahal"){
    ...
    for(i in 1:length(frontierObject$balance)){
      ...
      if(is.null(estCall)){
        m1 <- lm(myform,
           data=dataset[!(rownames(dataset) %in% frontierObject$drops[dropseq]),])
  ...
  if(frontierObject$metric=="L1"){
    ...
    for(i in 1:length(frontierObject$drops)){
      ...
      if(is.null(estCall)){
        m1 <- lm(myform,
           data=dataset[!(rownames(dataset)  %in% frontierObject$drops[1:i]),])
...
generateDataset <- function(finalFrontierObject, dataset, number.dropped){
  ...
  if(finalFrontierObject$metric=="Mahal"){
